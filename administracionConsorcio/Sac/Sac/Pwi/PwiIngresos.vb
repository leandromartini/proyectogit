...
Public Class PwiIngresos
    ...
    Public Function actualizarIngresosMes(ByVal auxDatagid As DataGridView, ByVal mes As String, ByVal año As String) As Boolean
        ...
        Try
            ...
            For Each row As DataGridViewRow In auxDatagid.Rows
                actualizarIngresosMes = brlIngresos.actualizarIngresosMes(row.Cells("id_uf").Value, 2, año, row.Cells("expMes").Value, row.Cells("expExtra").Value, row.Cells("mantEdif").Value, row.Cells("subTotal").Value, row.Cells("redondeo").Value, row.Cells("total").Value)
